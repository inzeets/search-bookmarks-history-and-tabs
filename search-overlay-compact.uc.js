...
(function unifiedSearchCompact() {
    ...
    const SHORTCUT = { key: 'c', alt: true, ctrl: false, shift: false };
    ...
    style.textContent = `
        ...
        #usc-scroll { flex: 1; overflow-y: scroll; }
        ...
        /* type stripe */
        #usc-table td:nth-child(1) { width: 4px; padding: 0; }
        /* favicon */
        #usc-table td:nth-child(2) { width: 16px; padding-left: 7px; padding-right: 7px; }
        #usc-table td:nth-child(2) img { width: 16px; height: 16px; display: block; }
        #usc-table td:nth-child(2):empty::before {
            content: '';
            display: block;
        ...
        /* combined cell: title | badge | tags | url */
        #usc-table td:nth-child(3) { padding-left: 2px; }
        .usc-cell-flex {
            display: flex;
            align-items: center;
            gap: 6px;
        ...
        .usc-tag-match  { color: #5dc8f5; font-weight: 600; }
        .usc-tag-rest   { color: #557a99; }
        .usc-text-match { color: #ffcf8a; font-weight: 600; }

        #usc-table tr.sel { background: #3c2711; color: #ffcf8a; text-shadow: 0 0 2px #000; }
        #usc-table tr.sel .usc-time-badge  { background: #1a3a0d; color: #a0e060; }
        #usc-table tr.sel .usc-tag-badge   { background: #0c1e30; }
        #usc-table tr.sel .usc-tag-rest    { color: #7aaabb; }

        .usc-tab      td:nth-child(1) { background: #ffffff55; }
        .usc-bookmark td:nth-child(1) { background: #44aa4488; }
        .usc-history  td:nth-child(1) { background: #2f7cce88; }
    `;
    ...
    function makeTagBadge(tag, tagTerms) {
        ...
        if (matchTerm) {
            ...
            if (pre)  { const s = doc.createElement('span'); s.className = 'usc-tag-rest';  s.textContent = pre;  span.appendChild(s); }
            const m = doc.createElement('span'); m.className = 'usc-tag-match'; m.textContent = hit; span.appendChild(m);
            if (post) { const s = doc.createElement('span'); s.className = 'usc-tag-rest';  s.textContent = post; span.appendChild(s); }
        } else {
    ...
    function render(items, tagTerms, textTerm) {
        ...
        for (const item of items) {
            ...
            // combined cell: [title~40ch] [time-badge] [tags] [url...]
            const combinedTd = doc.createElement('td');
            const flex = doc.createElement('div');
            flex.className = 'usc-cell-flex';

            const titleSpan = doc.createElement('span');
            titleSpan.className = 'usc-title-text';
            ...
            urlSpan.className = 'usc-url-text';
            urlSpan.appendChild(makeHighlighted(item.url, textTerm));
            flex.appendChild(urlSpan);

            combinedTd.appendChild(flex);

            tr.append(typeTd, iconTd, combinedTd);
            frag.appendChild(tr);
        }
        tbody.textContent = '';
